...
public class BlockClientPlayerRightClick extends BlockStaticPressurePlate {
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		...
		if (worldIn.isRemote && entityIn == AquaRegia.proxy.getClientPlayer() && worldIn.getTotalWorldTime() % 10 == 0) {
			// Make the player right click
			entityIn.addChatMessage(new TextComponentTranslation("message.aquaregia:client_player_right_click.right_click"));
			AquaRegia.proxy.doClientRightClick();
		}
	}
}
