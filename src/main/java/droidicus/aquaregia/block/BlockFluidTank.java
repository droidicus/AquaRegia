...
import droidicus.aquaregia.network.MessageFluidTankContents;
	...
	public static List<ITextComponent> getFluidDataForDisplay(IFluidTankProperties[] fluidTankProperties) {
		...
		for (final IFluidTankProperties properties : fluidTankProperties) {
			...
			if (fluidStack != null && fluidStack.amount > 0) {
				hasFluid = true;
				data.add(new TextComponentTranslation("tile.aquaregia:fluid_tank.fluid.desc", fluidStack.getLocalizedName(), fluidStack.amount, properties.getCapacity()));
			}
		}

		if (!hasFluid) {
			data.add(new TextComponentTranslation("tile.aquaregia:fluid_tank.empty.desc"));
		}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, @Nullable ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (fluidHandler != null) {
			...
			if (!worldIn.isRemote && (success || hand == EnumHand.OFF_HAND)) {
				AquaRegia.network.sendTo(new MessageFluidTankContents(fluidHandler.getTankProperties()), (EntityPlayerMP) playerIn);
			}
