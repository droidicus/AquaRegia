 ...
 * @author Choonster
 */
public class ItemClearer extends ItemAquaRegia {
	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack stack, World world, EntityPlayer player, EnumHand hand) {
		if (!world.isRemote) {
			...
			if (player.isSneaking()) {
				...
				player.addChatComponentMessage(new TextComponentTranslation("message.aquaregia:clearer.mode.%s", newMode));
			} else {
				final int minX = MathHelper.floor_double(player.posX / 16) * 16;
				final int minZ = MathHelper.floor_double(player.posZ / 16) * 16;

				player.addChatComponentMessage(new TextComponentTranslation("message.aquaregia:clearer.clearing", minX, minZ));
				...
				player.addChatComponentMessage(new TextComponentTranslation("message.aquaregia:clearer.cleared"));
			}
		}
