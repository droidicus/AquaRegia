 ...
 * @author Choonster
 */
public class ItemRitualChecker extends ItemAquaRegia {
	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn, EnumHand hand) {
		if (!worldIn.isRemote) {
			...
			if (invalidPosition.isPresent()) {
				...
				textComponent = new TextComponentTranslation("message.aquaregia:ritual_checker.failure", pos.getX(), pos.getY(), pos.getZ());
				textComponent.getStyle().setColor(TextFormatting.RED);
			} else {
				textComponent = new TextComponentTranslation("message.aquaregia:ritual_checker.success");
